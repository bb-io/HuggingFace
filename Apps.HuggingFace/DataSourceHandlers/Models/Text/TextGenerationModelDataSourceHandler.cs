...
public class TextGenerationModelDataSourceHandler : BaseInvocable, IAsyncDataSourceHandler
{
    public TextGenerationModelDataSourceHandler(InvocationContext invocationContext) : base(invocationContext)
    {
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var endpoint = $"/models?search={context.SearchString ?? ""}&sort=likes&direction=-1&filter=text-generation&limit=30";
        ...
        return models.Where(model => model.PipelineTag == "text-generation")
